...
* **Nightly Seeing Variations:** Fluctuating atmospheric conditions alter the point spread function (PSF) from night to night. A bright star might saturate the detector under
* excellent seeing conditions (where light is tightly focused), but remain unsaturated during poor seeing, creating a high-dispersion pseudo-variability effect.
* **Detector Geometry:** The VIRCAM instrument comprises an array of 16 distinct detector chips (petals). Because each petal has slightly different quantum efficiencies and saturation
* thresholds, observations of the same star tracking across different petals can trigger sporadic saturation flags.
